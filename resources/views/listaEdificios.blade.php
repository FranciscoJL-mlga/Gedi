        ...
        <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-sm">
            <thead>
                <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    ...
                    <th class="py-3 px-6 text-left">Despachos</th>
            ...
            <tbody class="text-gray-700 text-sm">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="py-3 px-6">{{$item->idEdi}}</td>
                    <td class="py-3 px-6">{{$item->nombre}}</td>
                    <form action="{{route("departamentos.actualizar",['departamento'=>$item->pivot->idDep, 'edificio'=>$item->idEdi])}}" method="post">
                        @csrf

                        <td class="py-3 px-6"><input type="number" name="despachos" value="{{ $item->pivot->despachos }}"></td>
                        <td class="py-3 px-6">
                    ...
                    <td class="py-3 px-6">  
                        <form action="{{route("departamentos.borrar",['departamento'=>$item->pivot->idDep, 'edificio'=>$item->idEdi])}}" method="post">
                            @csrf
